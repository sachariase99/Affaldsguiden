    ...
    <div className="pt-16 pb-8 mb-64 mx-8">
      ...
      <div className="mt-8 grid grid-cols-1 xl:grid-cols-2 gap-16">
        {sections.map((section) => (
          ...
          <div className="grid grid-cols-3 text-white shadow rounded-l-lg" style={{backgroundColor: `#${section.color}`}}>
            <h2 className="col-span-2 flex items-end p-8 text-2xl font-semibold">{section.title}</h2>
            <img className="col-span-1 w-full" src={section.image_url} alt={section.title} />
          </div></Link>
        ))}
      </div>
    </div>
